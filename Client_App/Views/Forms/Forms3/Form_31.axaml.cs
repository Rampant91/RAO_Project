...
public partial class Form_31 : BaseWindow<Form_31VM>
{
    private readonly Form_31VM _vm = null!;
    public Form_31VM? VM => DataContext as Form_31VM;
    ...
    {
        var command = new NewCopyExecutorDataAsyncCommand((Form_31VM)DataContext);
    ...
    {
        if (DataContext is not Form_31VM vm) return;
    ...
    private async Task CheckPeriod(Form_31VM vm)
    {
    ...
    private async Task RemoveEmptyForms(Form_31VM vm)
    {
        var desktop = (Application.Current?.ApplicationLifetime as IClassicDesktopStyleApplicationLifetime)!;
        List<Form> formToDeleteList = [];
        var lst = vm.Report[vm.FormType].ToList<Form31>();
