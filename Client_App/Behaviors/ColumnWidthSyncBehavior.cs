...
namespace Client_App.Behaviors
{
    public class ColumnWidthSyncBehavior : Behavior<Grid>
    {
        ...
        private void SyncColumns()
        {
            ...
            for (int i = 0; i < SourceDataGrid.Columns.Count; i++)
            {
        ...
        private void UpdateColumnWidth(int columnIndex)
        {
            ...
            if (actualWidth > 0)
            {
                AssociatedObject.ColumnDefinitions[columnIndex].Width = new GridLength(actualWidth - 1);//вычитаем 1 пиксель, иначе шапка таблицы съезжает
            }
        }
