...
public static class CategoryProvider
{
    ...
    [
        new() { Code = 1, Description = "Чрезвычайно опасно для человека (A/D >= 1000)" },
        new() { Code = 2, Description = "Очень опасно для человека (10 <= A/D < 1000)" },
        new() { Code = 3, Description = "Опасно для человека (1 <= A/D < 10)" },
        new() { Code = 4, Description = "Опасность для человека маловероятна (0,01 <= A/D < 1)" },
        new() { Code = 5, Description = "Опасность для человека очень маловероятна (A/D < 0,01)" }
    ];
