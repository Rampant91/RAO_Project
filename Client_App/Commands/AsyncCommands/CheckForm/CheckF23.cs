...
public class CheckF23 : CheckBase
{
    ...
    public static async Task<List<CheckError>> Check_Total(object? parameter)
    {
        ...
        foreach (var _ in db2.ReportsCollectionDbSet
            ...
            .Where(x => x.Master_DB.Rows20.Any(y => y.RegNo_DB == form20RegNo))
            .Include(x => x.Report_Collection)
            .Include(x => x.Report_Collection.Where(y => y.Year_DB == repYear && y.FormNum_DB == "2.2")).ThenInclude(report => report.Rows22)
            .Include(x => x.Report_Collection.Where(y => y.Year_DB == repYear && y.FormNum_DB == "2.2")).ThenInclude(x => x.Rows210))
