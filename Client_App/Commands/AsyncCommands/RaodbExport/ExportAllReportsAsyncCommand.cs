...
public partial class ExportAllReportsAsyncCommand : ExportRaodbBaseAsyncCommand
{
    public override async Task AsyncExecute(object? parameter)
    {
        ...
        await Task.Run(async () =>
        {
            await Parallel.ForEachAsync(reportsIdArray, parallelOptions, async (repsId, parallelCts) =>
            {
                ...
                await Task.Run(() => CopyTempFile(fullPath, fullPathTmp), parallelCts);
    ...
    /// <summary>
    /// Копирует файл из временной папки в папку назначения
    /// </summary>
    /// <param name="fullPath">Путь к файлу назначения</param>
    /// <param name="fullPathTmp">Путь к временному файлу</param>
    private static async Task CopyTempFile(string fullPath, string fullPathTmp)
    {
        try
        {
