...
namespace Models
{
    ...
    public class Form24 : Abstracts.Form2
    {
        ...
        public int ExcelRow(ExcelWorksheet worksheet, int Row,int Column,bool Transpon=true)
        {
            ...
            worksheet.Cells[Row + (Transpon == false ? 10 : 0), Column + (Transpon == true ? 10 : 0)].Value = MassTransferredToAnother_DB.Replace(" ", "") == "-" || MassTransferredToAnother_DB.Replace(" ", "") == "" || MassTransferredToAnother_DB.ToLower().Equals("без упаковки") || MassTransferredToAnother_DB.ToLower().Equals("прим.") ? MassTransferredToAnother_DB : Convert.ToDouble(MassTransferredToAnother_DB.Replace("е", "E").Replace("(", "").Replace(")", "").Replace("Е", "E").Replace(".", ","));
            worksheet.Cells[Row + (Transpon == false ? 11 : 0), Column + (Transpon == true ? 11 : 0)].Value = QuantityTransferredToAnother_DB;
            worksheet.Cells[Row + (Transpon == false ? 12 : 0), Column + (Transpon == true ? 12 : 0)].Value = MassRefined_DB.Replace(" ", "") == "-" || MassRefined_DB.Replace(" ", "") == "" || MassRefined_DB.ToLower().Equals("без упаковки") || MassRefined_DB.ToLower().Equals("прим.") ? MassRefined_DB : Convert.ToDouble(MassRefined_DB.Replace("е", "E").Replace("(", "").Replace(")", "").Replace("Е", "E").Replace(".", ","));
            worksheet.Cells[Row + (Transpon == false ? 13 : 0), Column + (Transpon == true ? 13 : 0)].Value = QuantityRefined_DB;
            worksheet.Cells[Row + (Transpon == false ? 14 : 0), Column + (Transpon == true ? 14 : 0)].Value = MassRemovedFromAccount_DB.Replace(" ", "") == "-" || MassRemovedFromAccount_DB.Replace(" ", "") == "" || MassRemovedFromAccount_DB.ToLower().Equals("без упаковки") || MassRemovedFromAccount_DB.ToLower().Equals("прим.") ? MassRemovedFromAccount_DB : Convert.ToDouble(MassRemovedFromAccount_DB.Replace("е", "E").Replace("(", "").Replace(")", "").Replace("Е", "E").Replace(".", ","));
