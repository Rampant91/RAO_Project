...
public class AutoCompleteBoxCategoryValidationBehavior : Behavior<AutoCompleteBox>
{
    private string? _originalValue;
    private bool _valueSelectedFromDropDown;

    public static readonly StyledProperty<ICollection<short?>?> ValidCategoriesProperty =
        Avalonia.AvaloniaProperty.Register<AutoCompleteBoxCategoryValidationBehavior, ICollection<short?>?>(
            nameof(ValidCategories),
            defaultValue: null);

    public ICollection<short?>? ValidCategories
    {
        get => GetValue(ValidCategoriesProperty);
        set => SetValue(ValidCategoriesProperty, value);
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(categoryStr) || ValidCategories == null)
            return false;
