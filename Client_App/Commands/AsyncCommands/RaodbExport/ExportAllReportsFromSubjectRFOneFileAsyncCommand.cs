...
/// <summary>
/// Экспорт всех организаций из конкретного региона в один файл .RAODB (отключён)
/// </summary>
public partial class ExportAllReportsFromSubjectRFOneFileAsyncCommand : ExportRaodbBaseAsyncCommand
{
    public override async Task AsyncExecute(object? parameter)
    {
        ...
        try
        {
            //Запрашиваем код субъекта
            var owner = Desktop.MainWindow;
            await Dispatcher.UIThread.InvokeAsync(async () =>
             { 
                 var dialog = new AskSubjectRFMessage();
                 codeSubjectRF = await dialog.ShowDialog<string?>(owner);
             });
        }
        catch (Exception ex)
        {
        ...
        try
        {
            countReports = dbReadOnly.ReportsCollectionDbSet
                .AsNoTracking()
                .AsSplitQuery()
                .AsQueryable()
                ...
                .AsEnumerable()
                .Where(x => x.Master_DB != null
                && x.Master_DB.RegNoRep != null
                && x.Master_DB.RegNoRep.Value.StartsWith(codeSubjectRF))
                .Count();
        }
        catch (Exception ex)
        {
        ...
        //Получаем список организаций, у которых РегНомер начинается с codeSubjectRF
        var reportsIdArray = dbReadOnly.ReportsCollectionDbSet
            .AsNoTracking()
            .AsSplitQuery()
            .AsQueryable()
            .Include(x => x.DBObservable)
