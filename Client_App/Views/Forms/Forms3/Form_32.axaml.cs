...
public partial class Form_32 : BaseWindow<Form_32VM>
{
    private readonly Form_32VM _vm = null!;
    public Form_32VM? VM => DataContext as Form_32VM;
    ...
    {
        var command = new NewCopyExecutorDataAsyncCommand((Form_32VM)DataContext);
    ...
    {
        if (DataContext is not Form_32VM vm) return;
            ...
            case "Нет":
                {
                    ...
                    if (vm.FormType is not "1.0" and not "2.0" and not "3.0")
                    {
    ...
    private async Task CheckPeriod(Form_32VM vm)
    {
        var desktop = (Application.Current?.ApplicationLifetime as IClassicDesktopStyleApplicationLifetime)!;
        if (vm.Report.FormNum_DB is "1.0" or "2.0" or "3.0") return;
        var reps = vm.Reports;
    ...
    private async Task RemoveEmptyForms(Form_32VM vm)
    {
        var desktop = (Application.Current?.ApplicationLifetime as IClassicDesktopStyleApplicationLifetime)!;
        List<Form> formToDeleteList = [];
        var lst = vm.Report[vm.FormType].ToList<Form32>();
