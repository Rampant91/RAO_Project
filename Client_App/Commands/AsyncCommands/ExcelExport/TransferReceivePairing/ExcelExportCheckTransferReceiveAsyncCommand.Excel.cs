...
public partial class ExcelExportCheckTransferReceiveAsyncCommand
{
    ...
    private static void CreateLegendSheet(ExcelPackage excelPackage)
    {
        ...
        ColorRow(PairingFieldNearFill, "Жёлтый", "Небольшое отличие: опечатка в одном знаке, дата в пределах ±15 дней, непарный код приёма/передачи, похожие на вид символы, разные написания одного номера или типа и т.п.");
        ...
        Body("Если паспорт и заводской номер пустые (или вместо них стоят «-», «б.н.» и т.п.), несколько строк могут относиться к одной партии: при поиске пары сравнивается суммарное количество. В подсветке ближайшего совпадения одинаковые пустые значения (оба «-» или оба «б.н.») — зелёные; «пусто напротив заполненного» — красные.");
