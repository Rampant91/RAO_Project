...
{
    public class ReportsToReports20_Converter : IValueConverter
    {
        public object Convert(object Value, Type tp, object Param, CultureInfo info)
        {
            if (Value != null)
            {
                ...
                try
                {
                    if (rps_coll.Count != 0)
                    {
                        return new ObservableCollectionWithItemPropertyChanged<IKey>(rps_coll.GetEnumerable().Where(x => ((Reports)x).Master_DB.FormNum_DB == "2.0"));
                    }
                    else
                    {
