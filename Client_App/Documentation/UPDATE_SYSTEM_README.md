...
- **«Напомнить позже»** — не показывать ту же версию снова ~сутки (throttle только по этой кнопке)
...
- «Напомнить позже» откладывает только уже предложенный `releaseId` ~на сутки (не при одном показе)
- Сбой MpzfUpdater → `.mpzf-update\last-error.txt`, сообщение при следующем старте
