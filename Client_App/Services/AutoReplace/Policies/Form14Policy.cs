    ...
    public static void ApplyOperationCode(Form14 form)
    {
        ...
        switch (opCode)
        {
            ...
            case "11":
                ...
                if (DateOnly.TryParse(form.OperationDate_DB, new CultureInfo("ru-RU", false), out var operationDate) && form.DocumentDate_DB != operationDate.ToShortDateString()) form.DocumentDate.Value = operationDate.ToShortDateString();
                if (!string.IsNullOrWhiteSpace(masterOkpo) && form.ProviderOrRecieverOKPO_DB != masterOkpo) form.ProviderOrRecieverOKPO.Value = masterOkpo;
                if (form.TransporterOKPO_DB != dash) form.TransporterOKPO.Value = dash;
                break;
