...
namespace Models
{
    ...
    public class Form27 : Abstracts.Form2
    {
        ...
        public int ExcelRow(ExcelWorksheet worksheet, int Row,int Column,bool Transpon=true)
        {
            ...
            worksheet.Cells[Row + (Transpon == false ? 3 : 0), Column + (Transpon == true ? 3 : 0)].Value = FactedWasteValue_DB.Replace(" ", "") == "-" || FactedWasteValue_DB.Replace(" ", "") == "" || FactedWasteValue_DB.ToLower().Equals("без упаковки") || FactedWasteValue_DB.ToLower().Equals("прим.") ? FactedWasteValue_DB : Convert.ToDouble(FactedWasteValue_DB.Replace("е", "E").Replace("(", "").Replace(")", "").Replace("Е", "E").Replace(".", ","));
            worksheet.Cells[Row + (Transpon == false ? 4 : 0), Column + (Transpon == true ? 4 : 0)].Value = WasteOutbreakPreviousYear_DB.Replace(" ", "") == "-" || WasteOutbreakPreviousYear_DB.Replace(" ", "") == "" || WasteOutbreakPreviousYear_DB.ToLower().Equals("без упаковки") || WasteOutbreakPreviousYear_DB.ToLower().Equals("прим.") ? WasteOutbreakPreviousYear_DB : Convert.ToDouble(WasteOutbreakPreviousYear_DB.Replace("е", "E").Replace("(", "").Replace(")", "").Replace("Е", "E").Replace(".", ","));
            return 5;
        }
