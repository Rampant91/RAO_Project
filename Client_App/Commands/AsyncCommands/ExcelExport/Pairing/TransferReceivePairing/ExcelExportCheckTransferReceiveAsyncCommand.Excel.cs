...
public partial class ExcelExportCheckTransferReceiveAsyncCommand
{
    ...
    {
        CreateLegendSheet(excelPackage, _operationDateSearchToleranceDays);
    ...
    private static void CreateLegendSheet(
        ...
        int operationDateSearchToleranceDays = DefaultOperationDateSearchToleranceDays)
    {
        ...
        Bullet($"Программа ищет среди операций контрагента противоположной стороны (передача и приём) с датой операции в пределах ±{operationDateSearchToleranceDays} дней.");
        ...
        ColorRow(PairingFieldNearFill, "Жёлтый", $"Похоже, но не точное совпадение: опечатка (в т.ч. перестановка соседних знаков), ведущие нули, пропущенное тире в номере, хвост даты у номера, тип с уточнением в скобках, дата в пределах ±{operationDateSearchToleranceDays} дней, непарный код приёма/передачи, диапазон зав.№ напротив одиночного номера из ряда и т.п.");
        ...
        Bullet("Активность при таком перечислении: если у списка указана суммарная активность партии, а у одиночной строки — активность одного изделия, ячейка активности может быть жёлтой (чем лучше сходится «сумма ↔ одно изделие × число штук», тем ближе совпадение). Если активность одиночной строки не меньше суммы у списка — по этому правилу активность не подсвечивается.");
        Blank();
        ...
        Bullet($"Дата операции и дата измерения активности (1.4/1.6): для пары нужна одна и та же дата. Окно ±{operationDateSearchToleranceDays} дней только ограничивает поиск похожих строк; отличие в несколько дней справа — жёлтый.");
        Bullet($"Дата выпуска: для пары нужна одна и та же дата; в ближайшем совпадении отличие в пределах ±{operationDateSearchToleranceDays} дней — жёлтый (чем дальше, тем слабее совпадение).");
        ...
        Bullet("Проверьте ОКПО поставщика или получателя и код операции.");
